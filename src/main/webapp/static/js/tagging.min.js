...
!function (a, b, c, d) {
    ...
    e.prototype = {
        keys: {add: {comma: 188, enter: 13, spacebar: 32}, remove: {del: 46, backspace: 8}},
        defaults: {
            "case-sensitive": !1,
            "close-char": "&times;",
            "close-class": "tag-i",
        ...
        add: function (b) {
            ...
            for (f.config["case-sensitive"] || (b = b.toLowerCase()), e = h.length; e--;) if (g = b.indexOf(h[e]), g >= 0) return f.emptyInput(), i = f.config["forbidden-words-callback"], j = f.config["forbidden-words-text"], f.throwError(i, j, b);
            if (f.config["no-duplicate"]) for (e = f.tags.length; e--;) if (f.tags[e].pure_text === b) return f.emptyInput(), i = f.config["no-duplicate-callback"], j = f.config["no-duplicate-text"], f.throwError(i, j, b);
        ...
        init: function () {
            ...
            return d = this, d.config = a.extend({}, d.defaults, d.options, d.getDataOptions()), b = d.$elem.text(), d.$elem.empty(), d.$type_zone = a(c.createElement("input")).addClass(d.config["type-zone-class"]).attr("contenteditable", !0), d.$elem.addClass(d.config["tag-box-class"]).append(d.$type_zone), d.$type_zone.keydown(function (a) {
                ...
                if (g = d.getSpecialKeys(), h = d.config["forbidden-chars"], i = d.valInput(), f = a.which, i) {
                    for (e = h.length; e--;) if (c = i.indexOf(h[e]), c >= 0) return a.preventDefault(), i = i.replace(h[e], ""), d.focusInput(), d.valInput(i), j = d.config["forbidden-chars-callback"], k = d.config["forbidden-chars-text"], d.throwError(j, k, h[e]);
        ...
        remove: function (b) {
            ...
            if (b) for (e = c.tags.length; e--;) c.tags[e][0].innerHTML === b[0].innerHTML && c.tags.splice(e, 1); else b = c.tags.pop();
